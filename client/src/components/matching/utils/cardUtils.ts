...
let isHorizontalSwipe: boolean | null = null; // раз
...
export const startDrag = (
  ...
  onSwipe?: () => void,
  onSwipePositive?: () => void,
  onSwipeNegative?: () => void
) => {
  isDragging = true;
  ...
  const drag = (e: MouseEvent | TouchEvent) => {
    ...
    if (isHorizontalSwipe === null) {
      isHorizontalSwipe = Math.abs(dx) > Math.abs(dy); // три
    }
    ...
    if (isHorizontalSwipe) {
      ...
      if (Math.abs(dx) > 100) {
        ...
        setTimeout(() => {
          if (dx > 100) {
            onSwipePositive();
            console.log("SWIPE POSITIVE");
          }
          if (dx < -100) {
            onSwipeNegative();
            console.log("SWIPE NEGATIVE");
          }
          onSwipe();
          console.log("SWIPESWIPE");
